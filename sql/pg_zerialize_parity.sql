...
-- Infinite interval literals ('infinity'/'-infinity'::interval) are only
-- valid on PostgreSQL 17+; build the tested values conditionally so this
-- suite also runs on PostgreSQL 16.
DO $$
DECLARE
    style text;
    value interval;
    ...
    fixed_row pgz_parity_fixed_arrays;
BEGIN
    IF current_setting('server_version_num')::int >= 170000 THEN
        ...
        interval_bounds := ARRAY['-infinity'::interval, '0'::interval, 'infinity'::interval];
    ELSE
        interval_values := ARRAY[
            '0'::interval,
            '1 mon -2 days 03:04:05.000006'::interval
        ];
        interval_bounds := ARRAY['0'::interval];
    END IF;
    ...
    FOREACH style IN ARRAY ARRAY['postgres', 'postgres_verbose', 'sql_standard', 'iso_8601'] LOOP
        ...
        FOREACH value IN ARRAY interval_values LOOP
    ...
    IF row_to_msgpack(fixed_row) <> row_to_msgpack_slow(fixed_row) THEN
